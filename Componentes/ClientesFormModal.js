...
const ClientesFormModal = ({ visible, onClose, onSave, cliente, title }) => {
  ...
  useEffect(() => {
    if (cliente) {
      setNomeCliente(cliente.nomeClientes || '');
      setCpf(cliente.CPF || '');
      setEmail(cliente.email || '');
      setTelefone(cliente.telefone || '');
      setDataNasc(new Date(cliente.dataNasc) || new Date());
    } else {
      setNomeCliente('');
      setCpf('');
      setEmail('');
        ...
        <View style={styles.modalContainer}>
          <ScrollView contentContainerStyle={styles.scrollContainer}>
            ...
            <TouchableOpacity style={styles.button} onPress={handleSave}>
              <Text style={styles.buttonText}>{cliente ? 'Salvar Alterações' : 'Cadastrar' }</Text>
            </TouchableOpacity>
            
            <TouchableOpacity style={[styles.button, styles.buttonCancel]} onPress={onClose}>
              <Text style={styles.buttonText}>{cliente ? 'Cancelar Alterações': 'Cancelar'}</Text>
            </TouchableOpacity>
          </ScrollView>
        </View>
      </View>
